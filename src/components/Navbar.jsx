            ...
            <div className='container flex items-center justify-between'>
                <a className='text-xl font-bold text-primary flex items-center' href='#hero'>
                    <span className='relative z-10'>
                        <span className='text-glow text-foreground'>Vinay's</span> Portfolio
                    </span>
                </a>
                ...
                <div className={cn("fixed inset-0 bg-background/95 background-blur-md z-40 flex flex-col items-center justify-center",
                    ...
                    <div className='flex flex-col text-xl space-y-8'>
                        {navItems.map((item) => (
                            <a key={item.href} href={item.href} className='text-foreground/80 hover:text-primary transition-colors duration-300' onClick={() => setisMenuOpen(false)}>
                                {item.name}
                            </a>
                        ))}
                    </div>
